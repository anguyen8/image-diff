        ...
        for right_bbox in right_predictions[:,:4]:
            if shapely.geometry.box(*right_bbox).contains(shapely.geometry.Point(*left_in_right)):
                if candidate_bbox is None:
                    ...
                    minimum_dist = torch.norm(left_in_right - torch.tensor((right_bbox[:2] + right_bbox[2:4]) / 2).to(device))
                    continue
                dist = torch.norm(left_in_right - torch.tensor((right_bbox[:2] + right_bbox[2:4]) / 2).to(device))
        ...
        for left_bbox in left_predictions[:,:4]:
            if shapely.geometry.box(*left_bbox).contains(shapely.geometry.Point(*right_in_left)):
                if candidate_bbox is None:
                    ...
                    minimum_dist = torch.norm(right_in_left - torch.tensor((left_bbox[:2] + left_bbox[2:4]) / 2).to(device))
                    continue
                dist = torch.norm(right_in_left - torch.tensor((left_bbox[:2] + left_bbox[2:4]) / 2).to(device))
                if dist < minimum_dist:
                    candidate_bbox = left_bbox
                    minimum_dist = dist
