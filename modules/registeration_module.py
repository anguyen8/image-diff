...
class FeatureRegisterationModule(nn.Module):
    ...
    def transform_matrix(self, batch):
        M_1_to_2 = []
        M_2_to_1 = []
        for i, (p1, p2) in enumerate(zip(batch["points1"], batch["points2"])):
            if p1 is not None:
                ...
                # M_1_to_2.append(batch["transfm2d_1_to_2"][i])
                # M_2_to_1.append(batch["transfm2d_2_to_1"][i])
                M_1_to_2.append(torch.eye(3).to(device))
                M_2_to_1.append(torch.eye(3).to(device))
        ...
        return M_1_to_2, M_2_to_1, transform_points_1_to_2, transform_points_2_to_1
