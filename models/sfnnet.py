...
class DeepEMD(nn.Module):
    ...
    def get_sfc(self, support):
        ...
        label_shot = label_shot.type(torch.LongTensor).to(device)

        with torch.enable_grad():
            for k in range(0, self.args.sfc_update_step):
                rand_id = torch.randperm(self.args.way * self.args.shot).to(device)
        ...
        if solver == 'opencv':  # use openCV solver
            for i in range(num_query):
                for j in range(num_proto):
                    ...
                    similarity_map[i, j, :, :] =(similarity_map[i, j, :, :])*torch.from_numpy(flow).to(device)
