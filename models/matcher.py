...
from utils_cyws.box_ops import box_cxcywh_to_xyxy, generalized_box_iou
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        if len(outputs.shape) == 3:
            ...
            out_bbox = outputs.flatten(0, 1)
        else:
            bs, num_queries = 1, outputs.shape[0]
            out_bbox = outputs
...
class Matcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        indices = [linear_sum_assignment(c[i]) for i, c in enumerate(C.split(sizes, -1))]
        return [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in indices], sizes
    

class HungarianMatcherOriginal(nn.Module):
    """This class computes an assignment between the targets and the predictions of the network
