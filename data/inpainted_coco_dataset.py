...
from utils_cyws.general import cache_data_triton
...
class InpatinedCocoDataset(Dataset):
    def __init__(self, path_to_dataset, split, method, image_transformation, change, machine="local"):
    ...
    def add_random_objects(self, image_as_tensor, item_index):
        ...
        annotation_mask = utils_cyws.general.coco_annotations_to_mask_np_array(
    ...
    def __base_getitem__(self, item_index):
        ...
        if random.random() < 0.5 or self.split == "test":
            ...
            image1_target_bbox_indexes = [annot['bbox_index'] for annot in annotations]
            image2_target_bbox_indexes = [annot['bbox_index'] for annot in annotations]
        else:
            image1_image_as_tensor = self.read_image_as_tensor(
                cache_data_triton(self.path_to_dataset, image1_image_path, self.machine)
            )
        ...
        return {
            ...
            "image1_target_bbox_indexes": image1_target_bbox_indexes,
            "image2_target_bbox_indexes": image2_target_bbox_indexes,
            # "image1_path": os.path.join(self.path_to_dataset, image1_image_path),
            # "image2_path": os.path.join(self.path_to_dataset, image2_image_path)
            "image1_path": None,
            "image2_path": None
        }
