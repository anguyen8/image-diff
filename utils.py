...
def create_batch_from_image_data(image_data):
    batch = {}
    all_keys = [
        "image1",
        "image2",
    ...
    for item in image_data:
        # image1
        batch["image1"].append(item[0])
        # image2
        batch["image2"].append(item[1])
        # registration strategy
        batch["registration_strategy"].append("2d")
        for key in ["depth1", "depth2", "intrinsics1", "intrinsics2", "position1", "position2", "rotation1", "rotation2", "transfm2d_1_to_2", "transfm2d_2_to_1"]:
            batch[key].append(None)

    return batch

def create_batch_from_image_path(image_data):
    batch = {}
    all_keys = [
        "image1",
        "image2",
    ...
    for item in image_data:
        # image1
        value = read_image_as_tensor(item[0])
        batch["image1"].append(value)
        # image2
        value = read_image_as_tensor(item[1])
        batch["image2"].append(value)
        # registration strategy
        batch["registration_strategy"].append("2d")
        for key in ["depth1", "depth2", "intrinsics1", "intrinsics2", "position1", "position2", "rotation1", "rotation2", "transfm2d_1_to_2", "transfm2d_2_to_1"]:
            batch[key].append(None)
    ...
    for i in range(len(batch["image1"])):
        if batch["registration_strategy"][i] == "3d":
            ...
            if batch["depth1"][i] is None and batch["depth2"][i] is None:
                batch["depth1"][i] = depth_predictor.infer(batch["image1"][i].unsqueeze(0).to(device)).squeeze()
                batch["depth2"][i] = depth_predictor.infer(batch["image2"][i].unsqueeze(0).to(device)).squeeze()
